...
### Activity view
Now lets define our activity view. Adds following HTML do body instead of dots.
...
We just created simple input and button to add a new todo a list of existing todos with remove button.
...
### View logic
Now add some view logic so our view can show tasks in content provider.
...
Selector `@` means current element. `$_attrIf` function adds HTML attribute `completed` only if task property `completed` is positive (eg.: true / 1 / non empty). `$__prop` function assings current item index (`@key`) to `task` element's object attribute so we can reference it later.
...
#### Toggle
When user clicks on `<li>` element we toggles item's `completed` state.

Notice how we are accessing task index. In binding we said that task's index should be binded to `<li>`'s property so now we can easily access it.
